...
bool CYdLidar::initialize() {
	...
	bool ret = true;
    if (!checkCOMMs()) {
        fprintf(stderr,"[CYdLidar::initialize] Error initializing YDLIDAR scanner because of a communication failure.\n");
        return false;
	}

    if (!checkStatus()) {
        fprintf(stderr, "[CYdLidar::initialize] Error initializing YDLIDAR scanner because of a failure in scan mode.\n");
    }

    if (!turnOn()) {
        fprintf(stderr, "[CYdLidar::initialize] Error initializing YDLIDAR scanner because the motor failed to start.\n");
	}
